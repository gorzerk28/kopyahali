...
async function fetchIstanbulPrayerTimings(dateKey) {
  const cacheKey = String(dateKey || "").trim();
  if (prayerTimesCache.has(cacheKey)) {
    return prayerTimesCache.get(cacheKey);
  }

  const sourceDate = cacheKey ? new Date(`${cacheKey}T00:00:00`) : new Date();
  const queryDate = formatDateForAladhan(sourceDate);
  const endpoint = `https://api.aladhan.com/v1/timingsByCity/${queryDate}?city=Istanbul&country=Turkey&method=13`;
  const response = await fetch(endpoint, { cache: "no-store" });
  if (!response.ok) {
    throw new Error("prayer-time-fetch-failed");
  }
...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname === "/api/prayer-times" && req.method === "GET") {
    const siteAuth = getSiteAuth(req);
    if (!siteAuth) {
      return sendJson(req, res, 401, { ok: false, error: "Authentication required" });
    }

    const dateKey = String(url.searchParams.get("date") || new Date().toISOString().slice(0, 10));
    const nextDate = new Date(`${dateKey}T00:00:00`);
    nextDate.setDate(nextDate.getDate() + 1);
    const nextDateKey = nextDate.toISOString().slice(0, 10);

    try {
      const timings = await fetchIstanbulPrayerTimings(dateKey);
      const tomorrowTimings = await fetchIstanbulPrayerTimings(nextDateKey);
      ...
      return sendJson(req, res, 502, { ok: false, error: "Prayer timings provider unavailable" });
    }
  }
